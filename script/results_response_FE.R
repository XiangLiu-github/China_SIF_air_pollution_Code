...
get_plot <- function(adata, acrop) {
  ...
  p1 <- ozone %>%
    ...
    scale_x_continuous(limits = ozone_range, expand = expansion(mult = c(0, 0.1))) +
    labs(subtitle = acrop)
  ...
  p2 <- AOD %>%
    ...
    ggside(x.pos = "bottom", collapse = "x") +
    scale_xsidey_continuous(labels = NULL) +
    scale_x_continuous(limits = AOD_range, expand = expansion(mult = c(0, 0.1))) +
    labs(subtitle = acrop)
