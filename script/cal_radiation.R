...
BaRAD <- tibble(
  ...
  data = map(year, function(ayear) {
    rast(str_c("../data_archive/BaRAD/BaRAD", ayear, ".nc")) %>%
      resample(mask) %>%
      mask(mask) %>%
      as.data.frame(xy = T) %>%
      pivot_longer(-c(x, y),
        names_sep = "_", names_to = c(NA, "name", "month"), names_transform = list(month = as.integer)
      ) %>%
      pivot_wider()
  }, .progress = T)
) %>%
...
saveRDS(BaRAD, "data/outputs/radiation/tidied_BaRAD.rds")
