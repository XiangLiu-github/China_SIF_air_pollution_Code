...
varss <-
  c(
    ...
    str_c("bin", 0:39), str_c("surface_", 1:9)
  )
...
fmls <- map(terms, ~ str_c("GOSIF_sum ~ AOD + AOD ^ 2 + ", .x, " + cloud + cloud ^ 2 + ", add_terms(c(str_c("bin", 0:39), str_c("surface_", 1:9)))) %>%
  as.formula())
