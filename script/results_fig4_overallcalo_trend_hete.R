...
aerosol <- qread("data/impacts_aerosol_hete_full.qs", nthreads = qn)
...
ozone <- qread("data/impacts_ozone_AOT40_hete_full.qs", nthreads = qn)
...
p1 <- joined %>%
  ...
  geom_text(aes(x = 2006, y = kcal_percapita_perday, label = "Claimed year of self-efficiency"), data = . %>% filter(year == 2019), inherit.aes = F, nudge_y = -11, color = "grey30", family = "Roboto Condensed", size = 5) +
  geom_hline(aes(yintercept = kcal_percapita_perday), data = . %>% filter(year == 2015), linetype = "dashed", color = "grey50") +
  geom_text(aes(x = 2006, y = kcal_percapita_perday, label = "Possible year of self-efficiency"), data = . %>% filter(year == 2015), inherit.aes = F, nudge_y = +11, color = "grey50", family = "Roboto Condensed", size = 5) +
  ...
  geom_col(aes(y = kcal_percapita_perday), fill = "#07617D", width = 0.5, color = "black") +
...
p2 <- joined %>%
  ...
  ungroup() %>%
  mutate(name = fifelse(str_detect(name, "our"), "Counterfactual scenario", "Observed scenario")) %>%
  ggplot(aes(ymax = ymax, ymin = ymin, xmax = 4, xmin = 3, fill = crop)) +
  facet_wrap2(~name, strip = strip_themed(text_x = map(c("#F9A828", "#07617D"), ~ element_text(colour = .x)))) +
  geom_rect() +
  geom_text(
    x = 3.5, aes(y = (ymax + ymin) / 2, label = sprintf("%0.0f", round(value, digits = 2))),
...
p3 <- joined %>%
  ...
  geom_text_repel(
    aes(label = sprintf("%0.2f", round(value, digits = 2))),
    data = . %>% slice_max(value, n = 1), color = "yellow",
    family = "Roboto Condensed", size = 5
  ) +
  ...
  geom_text_repel(aes(x = PM25, y = O3, label = year),
    ...
    min.segment.length = Inf, size = 5,
    # box.padding = 0.5,
    direction = "both", vjust = "top", hjust = "left"
  ) +
  scale_fill_gradientn(
    name = "Percentage change in calorie intake",
    limits = c(-1, 8),
    oob = squish,
    colours = c(PNWColors::pnw_palette("Shuksan2")[5], PNWColors::pnw_palette("Lake") %>% rev())
  ) +
  theme_half_open(16, font_family = "Roboto Condensed") +
  background_grid() +
  scale_x_continuous(
  ...
  theme(legend.key.width = unit(3.5, units = "lines")) +
...
ggsave("figures/hist_impact_fig4_hete.pdf", patch, width = 3, height = 2.5, scale = 4)
