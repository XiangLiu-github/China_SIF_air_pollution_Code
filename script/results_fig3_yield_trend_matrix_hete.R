...
p1 <- joined %>%
  ...
  geom_line(size = 1) +
  scale_color_manual(
    values = PNWColors::pnw_palette("Shuksan2", n = 4),
...
p2 <- joined %>%
  ...
  geom_text_repel(
    aes(label = sprintf("%0.2f", round(value, digits = 2))),
    data = . %>% group_by(crop) %>% slice_max(value, n = 1) %>% ungroup(),
    size = 4, color = "yellow",
    family = "Roboto Condensed"
  ) +
  geom_path(aes(x = PM25, y = O3, alpha = year),
    color = "red",
...
ggsave("figures/hist_impact_fig3_hete.pdf", patch, width = 3.5, height = 2.5, scale = 3)
