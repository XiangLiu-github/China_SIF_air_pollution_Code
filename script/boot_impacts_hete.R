...
f1 <- read_rds("data/boots_f1_hete.rds") %>%
  ...
  mutate(AOD_results = future_pmap(list(crop, fdata, vdata, coefs, aod_data), function(acrop, adata, avdata, coefs, aod_data) {
    ...
    rel_X <- cal_data %>%
      mutate(
        ...
        iAOD1 = AOD1 * irg_fraction, iAOD2 = AOD2 * irg_fraction,
    ...
    if (str_detect(acrop, "Rice")) {
      ...
      rel_X <- rel_X %>% select(AOD1, AOD2, iAOD1, iAOD2, vAOD1, vAOD2)
    }
...
qsave(impacts, "data/impacts_aerosol_hete.qs", nthread = qn)
...
impacts <- impacts %>%
  mutate(ozone_results = future_pmap(list(crop, fdata, vdata, coefs, ozone_data), function(acrop, adata, avdata, coefs, ozone_data) {
    ...
    if (str_detect(acrop, "Rice")) {
      ...
      rel_X <- rel_X %>% select(AOT401, iAOT401, vAOT401)
    }
...
qsave(impacts, "data/impacts_ozone_AOT40_hete.qs", nthread = qn)
