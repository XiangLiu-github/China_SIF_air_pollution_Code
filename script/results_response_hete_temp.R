...
p1 <- ozone %>%
  ...
  geom_xsidehistogram(
    aes(x = AOT40, y = ..ncount.., fill = maxtmp_mean),
    data = data,
...
p2 <- AOD_tbl %>%
  ...
  geom_hline(yintercept = 0, size = 1, linetype = "dashed") +
  geom_xsidehistogram(
    aes(x = AOD, y = ..ncount.., fill = maxtmp_mean),
    data = data,
    show.legend = F, alpha = 0.6, color = "black",
    size = 0.3, bins = 60, position = "identity", inherit.aes = F
  ) +
...
patch <- p1 + p2 + plot_spacer() +
  ...
  scale_color_manual(values = c("#B1DFF2", "#ED6A6B")) &
  scale_fill_manual(values = c("#B1DFF2", "#ED6A6B"))

ggsave("figures/response_hete_temp.pdf", patch, width = 2, height = 2.25, scale = 6)
