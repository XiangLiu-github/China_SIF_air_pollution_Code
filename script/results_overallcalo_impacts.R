...
Fengetal <- qread("data/impacts_ozone_Feng2022.qs", nthreads = qn) %>%
  unnest() %>%
  group_by(crop, year) %>%
  summarise(across(contains("%"), ~ weighted.mean(.x, fraction)), .groups = "drop") %>%
  rename_with(~ str_c("fen", .x), contains("%")) %>%
  mutate(crop_parent = fifelse(str_detect(crop, "Rice"), "Rice", crop)) %>%
  group_by(crop_parent, year) %>%
  summarise(across(contains("%"), mean), .groups = "drop")
...
p1 <- joined %>%
  ...
  alpha = 0.5, show.legend = F, color = NA, fill = PNWColors::pnw_palette("Cascades", n = 3)[2]
  ) +
  geom_ribbon(aes(
    ymax = yield + `aer95%` * yield + `fen95%` * yield,
    ymin = yield + `aer5%` * yield + `fen5%` * yield
  ),
  alpha = 0.2, show.legend = F, color = NA, fill = PNWColors::pnw_palette("Cascades", n = 3)[1]
  ) +
  ggborderline::geom_borderline(aes(y = yield + `aer50%` * yield + `ozo50%` * yield), size = 1, show.legend = F, linetype = "dashed", lineend = "round", color = PNWColors::pnw_palette("Cascades", n = 3)[2]) +
  ggborderline::geom_borderline(aes(y = yield + `aer50%` * yield + `fen50%` * yield), size = 1, show.legend = F, lineend = "round", bordersize = .6, color = PNWColors::pnw_palette("Cascades", n = 3)[1]) +
  geom_line(aes(y = yield), show.legend = F, color = PNWColors::pnw_palette("Cascades", n = 3)[3]) +
...
p2 <- joined %>%
  mutate(
    ...
    kcal_percapita_perday_fen = 0.44 * area * (yield + `aer50%` * yield + `fen50%` * yield) * 1e3 * n * (1 - w) * E / (population * 1e4) / 365,
    kcal_percapita_perday_fen_up = 0.44 * area * (yield + `aer95%` * yield + `fen95%` * yield) * 1e3 * n * (1 - w) * E / (population * 1e4) / 365,
    kcal_percapita_perday_fen_lw = 0.44 * area * (yield + `aer5%` * yield + `fen5%` * yield) * 1e3 * n * (1 - w) * E / (population * 1e4) / 365
  ) %>%
  group_by(year) %>%
  ...
  ggplot(aes(x = year)) +
  geom_col(aes(y = kcal_percapita_perday_fen), fill = PNWColors::pnw_palette("Cascades", n = 3)[1], width = 0.6, color = "black", alpha = 0.5) +
  geom_text(aes(y = kcal_percapita_perday_fen, label = str_c("Feng et al. \n", round(kcal_percapita_perday_fen))),
    data = . %>% filter(year == "Avrg."),
    family = "Roboto Condensed", nudge_x = 1.2,
    color = PNWColors::pnw_palette("Cascades", n = 3)[1]
  ) +
  geom_errorbar(aes(ymax = kcal_percapita_perday_fen_up, ymin = kcal_percapita_perday_fen_lw), width = 0.3) +
  geom_col(aes(y = kcal_percapita_perday_our), fill = PNWColors::pnw_palette("Cascades", n = 3)[2], width = 0.6, color = "black") +
  geom_text(aes(y = kcal_percapita_perday_our, label = str_c("Our. ", round(kcal_percapita_perday_our))),
    data = . %>% filter(year == "Avrg."),
    family = "Roboto Condensed", nudge_x = 1.2,
    color = PNWColors::pnw_palette("Cascades", n = 3)[2]
  ) +
  geom_errorbar(aes(ymax = kcal_percapita_perday_our_up, ymin = kcal_percapita_perday_our_lw), width = 0.3) +
  geom_col(aes(y = kcal_percapita_perday), fill = PNWColors::pnw_palette("Cascades", n = 3)[3], width = 0.6, color = "black") +
  geom_text(aes(y = kcal_percapita_perday, label = str_c("Obs. ", round(kcal_percapita_perday))),
    data = . %>% filter(year == "Avrg."),
    family = "Roboto Condensed", nudge_x = 1.2,
    color = PNWColors::pnw_palette("Cascades", n = 3)[3]
  ) +
...
ggsave("figures/hist_impact_calo.pdf", patch, width = 3, height = 2, scale = 3)
