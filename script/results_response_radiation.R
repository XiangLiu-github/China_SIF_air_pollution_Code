...
data <- get_data(varss = c("GOSIF_sum", "GR", "DR", "cloud", "AOD"))

models_pooled <- feols(c(GR, DirR, DR, DF) ~
...
models_indi <- feols(c(GR, DirR, DR, DF) ~
  cloud + cloud^2 + cloud^3 +
    AOD + AOD^2 + AOD^3 |
    x_y + year,
...
rads <- c("Global radiation", "Direct radiation", "Diffuse radiation", "Diffuse fraction")
...
ggsave("figures/response_radiation.pdf", patch, width = 2.3, height = 2, scale = 5)
